<script lang="ts">
  ...
  import LocalLivestream from "./LocalLivestream.svelte";
  ...
  let videoID = "";
  let channel = "";
  let localLivestreamUrl = `http://localhost:1701/hls_${position}/stream.m3u8`;
...
$: if ($ws.message?.type === "livestreamUpdate") {
    localLivestreamUrl = $ws.message?.data
...
<main>
    ...
    {:else if currentView === "twitch"}
        <Twitch {channel} />
    {:else if currentView === "localLivestream"}
        <LocalLivestream streamUrl={localLivestreamUrl} />
